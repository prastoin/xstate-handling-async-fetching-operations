...
import { createLoadUserDataMainMachine } from "@/machines/LoadUserDataMainMachine";
import type { LoadUserCartActorRef } from "@/machines/LoadUserCartMachine"
import type { LoadUserInformationActorRef } from "@/machines/LoadUserInformationMachine"

const loadUserDataInvokingMachines = createLoadUserDataMainMachine();

const { send: sendToMainMachine, state: loadUserDataMachineState } =
    useMachine(loadUserDataInvokingMachines);
...
function sendResetContextToMainMachine() {
    sendToMainMachine({
...
const loadUserCartActorRef = useActor(computed(() => loadUserDataMachineState.value.children.loadUserCartMachine as LoadUserCartActorRef))
const loadUserInformationActorRef = useActor(computed(() => loadUserDataMachineState.value.children.loadUserInformationMachine as LoadUserInformationActorRef))
      ...
      ">
        ...
        <div style="display: flex; flex-direction: column">
            ...
            <template v-else>
                ...
                <button style="margin: auto" @click="sendResetContextToMainMachine">Reset the machine</button>
            </template>
